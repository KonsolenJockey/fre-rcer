...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private rfcgenLexer lexer;
	private rfcgenParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public rfcgenLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new rfcgenLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public rfcgenParser getParser() {
		if (parser == null) {
			parser = new rfcgenParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "net::sf::rcer::rfcgen::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"net::sf::rcer::rfcgen::GenChecks","net::sf::rcer::rfcgen::Checks"};
