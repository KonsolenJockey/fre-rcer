...
/**
 * A wizard page that presents a summary before the actual actions are
 * performed.
 * 
 * @author vwegert
 * 
...
public class SummaryPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		context.bindValue(SWTObservables.observeSelection(checkboxPlugin), BeansObservables
				.observeValue(generatorSettings, "pluginProjectSelected"), //$NON-NLS-1$
		...
		context.bindValue(SWTObservables.observeSelection(checkboxFragmentWin32), BeansObservables
				.observeValue(generatorSettings, "win32FragmentSelected"), //$NON-NLS-1$
				new UpdateValueStrategy(), new UpdateValueStrategy());
		...
		context.bindValue(SWTObservables.observeSelection(checkboxExportBundles), BeansObservables
				.observeValue(generatorSettings, "bundleExportSelected"), //$NON-NLS-1$
				new UpdateValueStrategy(), new UpdateValueStrategy());
		...
		GridDataFactory.swtDefaults().grab(true, false).align(SWT.FILL, SWT.CENTER).applyTo(
				exportPathText);
		context.bindValue(SWTObservables.observeText(exportPathText, SWT.Modify), BeansObservables
				.observeValue(generatorSettings, "exportPath"), //$NON-NLS-1$
				new UpdateValueStrategy(), new UpdateValueStrategy());
	...
	/**
	 * Updates the state of the checkboxes so that the text and the state
	 * correspond to the selected files.
	 */
	...
	private void updateCheckbox(Button checkbox, String fileName, String projectName) {
		if (fileName.length() == 0) {
			...
			if (workspaceRoot.getProject(projectName).exists()) {
				checkbox.setText(MessageFormat.format(Messages.SummaryPage_ReplaceProjectLabel,
						projectName));
				checkbox.setSelection(false);
			} else {
				checkbox.setText(MessageFormat.format(Messages.SummaryPage_CreateProjectLabel,
						projectName));
